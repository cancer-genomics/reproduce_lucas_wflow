...
priors2 <- select(screening_priors, method, study,
    ...
    pivot_wider(id_cols=c(method, study, iter, statistic, y),
...
fig <- priors2 %>%
    ...
    mutate(method_study=droplevels(method_study),
           method_study=factor(method_study,
                               study_levels)) %>%
    ggplot(aes(sensitivity, specificity)) +
    geom_density_2d(aes(color=method_study), ##alpha=0.3,
                    bins=15) +
...
ppv %>%
    ...
    pivot_wider(id_cols=c(method_study, percentile, percent_change),
